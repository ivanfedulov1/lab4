...
function clickHandler(event) {
    ...
    if (classList.contains('digit')) {
        if (isResultCalculated) {
            // Сбрасываем экран, если результат уже был вычислен
            screen.textContent = event.target.textContent;
            isResultCalculated = false;
        } else {
            // Продолжаем добавление символов к текущему числу
            screen.textContent += event.target.textContent;
        }
        ...
        // Очищаем экран и устанавливаем флаг
        screen.textContent = '';
        isResultCalculated = false;
    } else if (classList.contains('result')) {
        // Вычисляем результат, обновляем экран и устанавливаем флаг
